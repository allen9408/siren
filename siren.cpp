...
static pj_status_t siren_codec(const char *filein, const char *fileout)
{
	pj_pool_t *pool;
...
#define T   file_msec_duration/1000, file_msec_duration%1000
	...
	// Create memory pool
	pool = pjmedia_endpt_create_pool(mept, "siren", 1000, 1000);
...
int main(int argc, char *argv[])
{
    ...
    pj_gettimeofday(&t0);
    status = siren_codec(argv[1], argv[2]);
    pj_gettimeofday(&t1);
